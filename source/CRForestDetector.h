...
public:
	...
	void detectPyramidhard(IplImage *img, std::vector<float>& scales, std::vector<float>& ratios,
		const char *imfile, std::priority_queue<PatchHardMining, std::vector<PatchHardMining>, LessThanPatchHardMining>& neg_examples,
		const char *posfile, std::vector<CvRect>& vBBox, std::vector<std::vector<CvPoint> >& vCenter, int max_neg_samples);
...
private:
	...
	void detectColorHardNeg(IplImage* img, std::priority_queue<PatchHardMining, std::vector<PatchHardMining>, LessThanPatchHardMining>& pos_bad_examples,
		std::vector<float>& ratios, const char* imfile, const char* filename, std::vector<CvRect>& vBBox, std::vector<std::vector<CvPoint> >& vCenter, int max_neg_samples);
